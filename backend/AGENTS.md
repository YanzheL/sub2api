...
## OVERVIEW
`backend/` is the Go application: server bootstrap, config loading, DI, handlers, services, repositories, generated Ent ORM, and SQL migrations.
...
│   ├── service/       # Business logic, gateway, billing, ops
...
| Routing / middleware | `internal/server/`, `internal/server/routes/`, `internal/server/middleware/` | Route mounting and auth middleware |
| Gateway logic | `internal/service/` | Multi-provider routing, billing, rate limiting |
